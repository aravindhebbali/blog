...
```{r str62}
# detect @
str_detect(mockdata$emails, pattern = "@")

# count @
str_count(mockdata$emails, pattern = "@")
```
...
```{r str63}
str_c("email id:", mockdata$emails)
```
...
```{r str65}
str_trunc(mockdata$emails, width = 10)
str_trunc(mockdata$emails, width = 10, side = "left")
str_trunc(mockdata$emails, width = 10, side = "center")
```
...
```{r str66}
str_sort(mockdata$emails)
str_sort(mockdata$emails, decreasing = TRUE)
```
...
```{r str69}
str_extract(mockdata$emails, pattern = "org")
```
...
```{r str70}
str_match(mockdata$emails, pattern = "org")
```
...
```{r str71}
str_which(mockdata$emails, pattern = "org")
```
...
```{r str72}
str_locate(mockdata$emails, pattern = "@")
```
...
- find the location of the first number present in the image url
...
#### Step 2: Find the location of the first number present in the image url.
...
- locate all occurences of the pattern `/`
- extract the location of the third `/` in the url
- extract the part of the string upto the above location
...
#### Step 1: Locate all occurences of the pattern `/`.
...
#### Step 2: Extract the location of the third `/` in the url.
...
#### Step 3: Extract the part of the string upto the above location.
...
#### Step 4: Split using the pattern `\\.`.
...
#### Step 5: Extract the first element.
...
#### Step 6: Split using the pattern `//`.
...
#### Step 7: Extract the second element.
