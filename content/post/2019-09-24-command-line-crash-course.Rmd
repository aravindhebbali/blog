...
In this tutorial, you will be introduced to shell commands. We have selected a set of commands we think will be useful in general to a wide range of audience.
...
knitr::include_graphics("mac-terminal.png")
```
...
Note, the terminal depends on the underlying operating system. To learn more about the RStudio terminal, read this [article](https://support.rstudio.com/hc/en-us/articles/115010737148-Using-the-RStudio-Terminal) or watch this [webinar](https://resources.rstudio.com/webinars/terminal-updates). In this tutorial, we will use the RStudio terminal on [RStudio Cloud](https://rstudio.cloud/) to ensure that all users have access to Linux bash. You can try all the commands used in this tutorial on your local system as well except in case of Windows users.
...
`man` is used to view the system's reference manual. Let us use it to view the documentation of the `ls` command which we will use in the next section. 
...
`pwd` displays the name of the current **p**resent **w**orking **d**irectory. 
...
How about making copies of folders? Use the `-r` option to copy entire folders. Let us create a copy of the `r` folder and name it as `R`. The `-r` option stands for `--recursive` i.e. copy directories recursively. 
...
We will look into the `cat` command in more detail in the next section but for the time being it is sufficient to know that it prints contents of a file. The file contains release names of different R versions.
...
Remove files in verbose mode, printing a message for each removed file. This is useful when you want to see the details of the files being removed. In the below example, we will remove all files with `.txt` extension from the `myfiles` folder. 
...
```{bash 340a, eval=FALSE}
ls
```

```{bash 340b, echo=FALSE}
cd cline
ls
```
...
The `head` command will display the firt 10 lines of a file(s) by default. It can be used to display the first few lines or bytes of a file. 
...
`find` powerful tool for working with the files. It can be used on its own to locate files, or in conjunction with other programs to perform operations on those files.
...
Let us search for all files with `.txt` extension in the `r` folder.
...
```{bash c115b, echo=FALSE, eval=FALSE}
cd cline
cat urls.txt
```
...
In this section, we will explore file compression using the following
...
In R, we can the [zip]() package has the functionalities to handle ZIP archives. The `tar()` and `untar()` functions from the `utils` package can handle GZIP archives.
...
system2(command = "date", stdout = TRUE)
```

Great! Now, how do we specify the arguments? The additional arguments of a 
...
The `-e` option allows us to specify R expression(s). `R -e` will launch R and then execute the code specified within quotes. Use semi-colon to execute multiple commands.
...
```{bash c99, echo=FALSE}
cp -r /mnt/j/R/rstudio-export /mnt/j/blog/content/post/cline
``` 
